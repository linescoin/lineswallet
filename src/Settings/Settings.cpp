...
const char DEFAULT_WALLET_FILE_NAME[] = "lines.wallet";
...
const quint64 DEFAULT_OPTIMIZATION_MIXIN = 1;
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.lines.pw:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("lineswallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("lineswallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("lineswallet.desktop");
  ...
  res = autorunSettings.contains("LinesWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("LinesWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "pw.lines.lineswallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Lines Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("LinesWallet", appPath);
    } else {
      autorunSettings.remove("LinesWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\lines", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:lines");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\lines\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\lines\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("lineswallet.cfg"));
